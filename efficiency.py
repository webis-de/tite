...
from transformers import AutoModel, AutoTokenizer, BertConfig, PreTrainedModel, PreTrainedTokenizerBase
...
def main(args=None):
    ...
    parser.add_argument("--num_texts", type=int, default=100_000)
    ...
    configs = [
        ("bert", BertConfig(hidden_act="gelu_pytorch_tanh")),
    ...
    for config_name, config in pg:
        ...
        for text_type, text, max_length in tqdm(
            ...
            _results[(config_name, text_type)] = model_results

    results = pd.DataFrame(_results).T.reset_index().rename(columns={"level_0": "model", "level_1": "text_type"})
