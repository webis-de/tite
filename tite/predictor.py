...
class BlockSentencePredictor(Module):
    ...
    def forward(self, hidden_states: Tensor, student_batch_idcs: tuple[int], *args, **kwargs) -> Tensor:
        ...
        repeated_hidden_states = padded_hidden_states.repeat_interleave(num_blocks, dim=0)
        ...
        for layer in self.layers:
            hidden_states, attention_mask = layer(repeated_hidden_states, attention_mask)
        pred_hidden_states = (hidden_states * attention_mask[:, :, None]).sum(1, keepdim=True) / attention_mask.sum(-1)[
            :, None, None
        ]
