...
class CharacterDelete(CharacterTransformation):
    ...
    #     delete_mask = torch.rand(encoded.shape) < self.delete_prob
...
class CharacterInsert(CharacterTransformation):
    ...
    #     insert_num = int(torch.distributions.Binomial(encoded.shape[0], self.insert_prob).sample().item())
...
class CharacterSwapNeighboring(CharacterTransformation):
    ...
    # def transform(self, text: str) -> list[str]:
    #     encoded = self.encode(text)
    #     swap_mask = torch.rand(encoded.shape[0] - 1) < self.swap_prob
    #     encodedtransformed = encoded.clone()
    #     encodedtransformed[:-1][swap_mask] = encoded[1:][swap_mask]
    #     encodedtransformed[1:][swap_mask] = encoded[:-1][swap_mask]
    #     transformed = self.decode(encodedtransformed)
    #     return transformed
...
class SentenceBlock(SentenceTransformation):
    ...
    def transform(self, text: str) -> list[str]:
        sentences = self.split(text)
        blocked_sentences = self.group_into_blocks(sentences)
