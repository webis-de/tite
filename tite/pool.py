...
def backward_pooling_kernel(
    ...
    Y = Y + Y_start_idx * stride_y_seq_len + tl.cdiv(pid_window, stride) * stride_y_dim

    if pid_m * BLOCK_M >= x_seq_len:
        return
    rm = pid_m * BLOCK_M + tl.arange(0, BLOCK_M)
    rk = tl.arange(0, BLOCK_K)
    ...
    for i in range(kernel_size):
        rm_ = tl.cdiv(rm[:, None] - i, stride)
        Y_ = Y + (rm_ * stride_y_seq_len + rk[None, :] * stride_y_dim)
        y = tl.load(Y_, mask=(rm_ >= 0) & (rm_ < y_seq_len) & (rk[None, :] < dim), other=0.0)
        acc += y

    acc /= kernel_size

    X = X + rm[:, None] * stride_x_seq_len + rk[None, :] * stride_x_dim
    tl.store(X, acc, mask=(rm[:, None] < x_seq_len) & (rk[None, :] < dim))
...
def apply_backward_pooling(
    ...
    BLOCK_M = 8 if dim <= 128 else 4
    BLOCK_K = 32 if dim <= 32 else (64 if dim <= 64 else (128 if dim <= 128 else 256))
    grid = lambda META: (triton.cdiv(x_max_seq_len, META["BLOCK_M"]), batch)  # noqa
