...
def main(args=None):
    ...
    head_dim = 64
    hidden_states = torch.rand(batch_size, seq_len, num_attention_heads, head_dim)
    _rope = _RotaryPositionalEmbeddings(head_dim, 512)
    rope = RotaryPositionalEmbeddings(head_dim, 512)

    _rope_hidden_states = _rope(hidden_states)
    rope_hidden_states = rope(hidden_states.transpose(1, 2)).transpose(1, 2)
